...
public class TIdentityExcelUtil {
    /**
     * 先创建一个实体类
     */
     ...
     * @return
     */
    public List<TIdentity> getExcelInfo(MultipartFile mFile) {
        //获取文件名
        String fileName = mFile.getOriginalFilename();
        List<TIdentity> ilist = null;
        try {
            //验证文件名是否合格
...
public List<TIdentity> createExcel(InputStream is , boolean isExcel2003){
        List<TIdentity> ilist = null;
        try {
            Workbook wb = null;
            if(isExcel2003){
...
public List<TIdentity> readExcelValue(Workbook wb){
        List<TIdentity> ilist=new ArrayList<>();
        //先得到一个sheet
		...
		String phone = String.valueOf(cell.getNumericCellValue());
                    tIdentity.setPhone(phone.substring(0,phone.length()-2>0?phone.length()-2:1));
                }

               else{
                //如果不是纯数字可以直接获得名称
                tIdentity.setPhone(cell.getStringCellValue());
            }} 
		if(c==1){
                   if(cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC){
    
		String identity = String.valueOf(cell.getNumericCellValue());
                    tIdentity.setIdentity(identity.substring(0,identity.length()-2>0?identity.length()-2:1));
                }

               else{
                //如果不是纯数字可以直接获得名称
                tIdentity.setIdentity(cell.getStringCellValue());
            }} }
            }
            //最后将这些全部添加到ilist中
            ilist.add(tIdentity);
