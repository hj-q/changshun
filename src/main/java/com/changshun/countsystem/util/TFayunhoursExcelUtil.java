...
public class TFayunhoursExcelUtil {
    /**
     * 先创建一个实体类
     */
     ...
     * @return
     */
    public List<TFayunhours> getExcelInfo(MultipartFile mFile) {
        //获取文件名
        String fileName = mFile.getOriginalFilename();
        List<TFayunhours> ilist = null;
        try {
            //验证文件名是否合格
...
public List<TFayunhours> createExcel(InputStream is , boolean isExcel2003){
        List<TFayunhours> ilist = null;
        try {
            Workbook wb = null;
            if(isExcel2003){
...
public List<TFayunhours> readExcelValue(Workbook wb){
        List<TFayunhours> ilist=new ArrayList<>();
        //先得到一个sheet
        ...
        for (int r = 1 ; r < totalRows; r++){
            ...
            TFayunhours tFayunhours = new TFayunhours();
            //循环xcel的列
		...
		if(c==1){
		 ...
		 Double hours = cell.getNumericCellValue();
                 tFayunhours.setHours(hours);}
             else{
                //如果不是纯数字可以直接获得名称
                tFayunhours.setHours(cell.getNumericCellValue());
            }
        } 
		if(c==2){
                   if(cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC){
		...
		String bumen = String.valueOf(cell.getNumericCellValue());
                    tFayunhours.setBumen(bumen.substring(0,bumen.length()-2>0?bumen.length()-2:1));
                }

               else{
                //如果不是纯数字可以直接获得名称
                tFayunhours.setBumen(cell.getStringCellValue());
            }} }
            }
            //最后将这些全部添加到ilist中
            ilist.add(tFayunhours);
        }        return ilist;
    }}
